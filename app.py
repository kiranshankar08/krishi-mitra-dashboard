...
# ---------------- GOOGLE SHEETS AUTH ----------------
# Reads credentials from Streamlit Cloud secrets (no credentials.json file needed)
scope = [
...
# ------------------------------------------------------------
# Instead of calling Flask on your laptop (which is now gone),
# this writes the override command (-1 / 0 / 1) directly into
# Google Sheets cell B1. The ESP32 reads that cell every 15
# seconds and acts on it. No laptop needed at all.
# ============================================================
def update_override(value):
    try:
        sheet.update_cell(1, 2, str(value))  # Row 1, Col B = override command
        if value == 1:
            st.success("✅ Command sent: PUMP ON")
        elif value == 0:
            st.success("✅ Command sent: PUMP OFF")
        else:
            st.success("✅ Command sent: AUTO MODE")
...
st.sidebar.info("💧 **Confidence-Weighted Dosing** scales pump-on duration by rain-forecast confidence instead of a binary skip, hedging against forecast error.")
